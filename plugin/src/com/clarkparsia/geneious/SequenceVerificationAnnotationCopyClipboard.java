...
public class SequenceVerificationAnnotationCopyClipboard extends SequenceAnnotationGenerator {
	...
	public String showCopyText(SequenceAlignmentDocument doc) {
		...
		JLabel label = new JLabel("<html>Sequence annotations found: " + annotations.size()
				+ "<br>Click OK to copy the following contents to clipboard.</html>");
		panel.add(label, BorderLayout.NORTH);
		panel.add(scrollPane, BorderLayout.CENTER);

		// int confirmation = JOptionPane.showConfirmDialog(null, panel, "Copy
		// annotations to clipboard", JOptionPane.OK_CANCEL_OPTION);
		// return confirmation == JOptionPane.OK_OPTION ? copyText : null;

		boolean confirmed = Dialogs.showOkCancelDialog(panel, "Copy annotations to clipboard", null,
				DialogIcon.NO_ICON);
	...
	private String computeCopyText(SublimeAlignmentDocument alignmentDoc, List<SequenceAnnotation> annotations) {
		...
		for (SequenceAnnotation annotation : annotations) {
			...
			Boolean isAmbiguous = GeneiousUtils
		...
		// SequenceAnnotation root =
		// GeneiousUtils.findRootAnnotation(alignmentDoc.getDesign());
		for (SequenceAnnotation component : components) {
			// SequenceAnnotation parent =
			// GeneiousUtils.findParentAnnotation(component, components);
			if (/* parent == null || parent == root && */ !GeneiousUtils.isVariant(component)
					&& !component.getIntervals().isEmpty()) {
	...
	private SequenceListSummary computeSummary(SublimeAlignmentDocument alignmentDoc,
			SequenceAnnotationInterval interval) {
		try {
			SequenceListOnDisk.Builder<SequenceDocument> builder = new SequenceListOnDisk.Builder<SequenceDocument>(
					false, Alphabet.NUCLEOTIDE, true);
