...
public class SBOLExporter extends DocumentFileExporter {
	...
	private static final Set<String> IGNORED_ANNOTION_TYPES = ImmutableSet.of(
			SequenceAnnotation.TYPE_EDITING_HISTORY_DELETION, SequenceAnnotation.TYPE_EDITING_HISTORY_INSERTION,
			SequenceAnnotation.TYPE_EDITING_HISTORY_REPLACEMENT);

	/**
	 * Constant SequenceAnnotation.INVISIBLE_ANNOTATION_TYPE_PREFIX is defined
	 * in Geneious version 5.6 but not in 5.5 so we redefine it here
	 */
	...
	public DocumentSelectionSignature[] getSelectionSignatures() {
		return new DocumentSelectionSignature[] {
				DocumentSelectionSignature.forNucleotideSequences(1, Integer.MAX_VALUE, true) };
	}

	@Override
	public void export(File file, PluginDocument[] docs, ProgressListener progress, Options options)
			throws IOException {
		export(file, Arrays.asList(docs), progress, options);
	}

	@Override
	public void export(File file, AnnotatedPluginDocument[] docs, ProgressListener progress, Options options)
			throws IOException {
	...
	private void export(File file, List<PluginDocument> docs, ProgressListener progress, Options options)
			throws IOException {
		...
		try {
			...
			for (int i = 0; i < count; i++) {
				...
				if (internalDoc instanceof SequenceDocument) {
					...
					SublimeAlignmentDocument alignmentDoc = new SublimeAlignmentDocument(
							(SequenceAlignmentDocument) internalDoc);
	...
	private void processDocument(SBOLDocument sbolDoc, SequenceDocument sequenceDoc,
		...
		Iterable<SequenceAnnotation> annotations = Iterables.filter(sequenceDoc.getSequenceAnnotations(),
				new Predicate<SequenceAnnotation>() {
					@Override
		...
		Map<SequenceAnnotation, org.sbolstandard.core2.SequenceAnnotation> components = Maps.newHashMap();
		// parent relationships for each annotation
		Map<SequenceAnnotation, SequenceAnnotation> parents = computeParents(annotations);
		// create a root component
		ComponentDefinition rootComponent = createRootComponent(sequenceDoc, components);

		for (SequenceAnnotation annotation : annotations) {
			processRegularAnnotation(annotation, components, parents, rootComponent, sequenceDoc.getCharSequence(),
					alignmentDoc);
		}

		for (SequenceAnnotation annotation : annotations) {
			org.sbolstandard.core2.SequenceAnnotation sbolAnn = components.get(annotation);
			for (SequenceAnnotationQualifier qualifier : annotation.getQualifiers()) {
				if (qualifier.getName().equals(GeneiousQualifiers.PRECEDES)) {
					...
					Predicate<org.sbolstandard.core2.SequenceAnnotation> uriFinder = SBOLPredicates.uri(uri);
					org.sbolstandard.core2.SequenceAnnotation precedes = SBOLObjectFinder.findObject(rootComponent,
							uriFinder, org.sbolstandard.core2.SequenceAnnotation.class);
					if (precedes != null) {
		...
		if (alignmentDoc != null) {
			...
			List<SequenceAnnotation> variantAnnotations = GeneiousUtils
					.getVariantAnnotations(alignmentDoc.getAnnotations());
			...
			for (SequenceAnnotation variantAnnotation : variantAnnotations) {
				...
				ComponentDefinition parentComponent = components.get(parent).getSubComponent();
	...
	private ComponentDefinition createRootComponent(SequenceDocument sequenceDoc,
			Map<SequenceAnnotation, org.sbolstandard.core2.SequenceAnnotation> components) {
		SequenceAnnotation rootAnnotation = GeneiousUtils.findRootAnnotation(sequenceDoc);
		ComponentDefinition rootComponent = null;
		if (rootAnnotation == null) {
			rootComponent = SublimeSBOLFactory.createComponentDefinition();
		} else {
	...
	private void processRegularAnnotation(SequenceAnnotation annotation,
			Map<SequenceAnnotation, org.sbolstandard.core2.SequenceAnnotation> components,
			Map<SequenceAnnotation, SequenceAnnotation> parents, ComponentDefinition rootComponent,
			SequenceCharSequence sequence, SublimeAlignmentDocument alignmentDoc) {
		if (components.containsKey(annotation)) {
			return;
		}

		ComponentDefinition parentComponent = null;
		...
		org.sbolstandard.core2.SequenceAnnotation ann = createSBOLAnnotation(annotation, alignmentDoc, parent);
	...
	private org.sbolstandard.core2.SequenceAnnotation createSBOLAnnotation(SequenceAnnotation annotation,
			SublimeAlignmentDocument alignmentDoc, SequenceAnnotation parent) {
		ComponentDefinition comp = SublimeSBOLFactory.createComponentDefinition();
		...
		org.sbolstandard.core2.SequenceAnnotation sbolAnnotation = SublimeSBOLFactory.createSequenceAnnotation();
	...
	private SublimeSequenceVariant createSBOLVariant(SequenceAnnotation annotation,
			SublimeAlignmentDocument alignmentDoc) {
		...
		sbolVariant.setAmbiguous(GeneiousUtils
				.mapAmbiguousVerification(annotation.getQualifierValue(GeneiousQualifiers.AMBIGUOUS_VERIFICATION)));
